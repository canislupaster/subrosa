...
export type PuzzleInputSchema = readonly Readonly<
	{ type: "number" | "string"; name: string; key: string }
>[];
export type PuzzleInput<T extends PuzzleInputSchema> = Readonly<
	{
		[K in T[number]["key"]]: "number" | "string" extends (T[number] & { key: K })["type"]
			? string | bigint
			: (T[number] & { key: K })["type"] extends "string" ? string
			: bigint;
	}
>;
...
export type PuzzleData = { name: string; key: string } & ({
	kind: "decode";
...
function simple<T extends PuzzleInputSchema>(
	t: T,
	x: Omit<SimplePuzzle<T>, "schema" | "kind">,
): SimplePuzzle<PuzzleInputSchema> {
...
export const puzzles = [{
	...
	key: "add",
	...simple(
		[{ type: "number", name: "x", key: "x" }, { type: "number", name: "y", key: "y" }] as const,
		{
			generator(seed) {
				const r = new RNG(seed);
				return { x: BigInt(r.nextRange(-200, 200)), y: BigInt(r.nextRange(-200, 200)) };
			},
			solve(inp) {
				return inp.x+inp.y;
			},
	...
	...simple(
		[{ type: "number", name: "x", key: "x" }, { type: "number", name: "y", key: "y" }] as const,
		{
			generator(seed) {
				...
				return { x: BigInt(div*r.nextRange(0, 200)+r.nextRange(0, div-1)), y: BigInt(div) };
			},
			solve(inp) {
				return inp.x/inp.y;
			},
	...
	name: "Reverse",
	key: "reverse",
	...simple([{ type: "string", name: "Input", key: "input" }] as const, {
	...
	key: "permutation",
	...simple([{ type: "string", name: "Input permutation", key: "input" }] as const, {
		generator(seed) {
	...
	encode(inp) {
		const key = "wang";
		return fill(inp.length, i => charAdd(inp.charAt(i), charToNum[key.charAt(i%key.length)]+i))
			.join("");
	},
}, {
	name: "Leapfrog",
	...
	encode(inp) {
		...
		for (; i+1 < inp.length; i++) {
			...
			while (
				i+1-j < alphaLen && i+1 < inp.length
				&& (charToNum[inp[i+1]]-charToNum[inp[i]]-d)%alphaLen == 0
			) {
				i++;
			}
