...
export type Register = Readonly<
	({
		type: "value";
		value: bigint | string; // initial values, determined by program state at runtime
	} | { type: "param" }) & { name: string | null }
>;
...
export type Node = Readonly<
	{ op: "inc" | "dec"; lhs: number } | {
		...
		lhs: number;
		rhs: number; // lhs +=,-=,= rhs, lhs = rhs[lhs]
	} | {
...
export type ProgramState = {
	...
	stats: ProgramStats;
	stack: { proc: number; registers: ReadonlyMap<number, RegisterRef>; i: number }[];
};
...
export function makeState(
	{ input, procs, entry, stopOnBreakpoint }: {
	...
	const pstate: ProgramState = {
		procs,
		outputRegister: { current: input[0] },
		stack: [],
		visitedNodes: new Map(),
		activeRegisters: new Set(),
...
export function makeEntryProc(puzzle: PuzzleData): Procedure {
	const regs = fill(puzzle.kind == "decode" ? 1 : puzzle.schema.length, i => {
		...
		return [
			i,
			{ name: i == 0 ? `${inpName} and output` : inpName, type: "param" } satisfies Register,
		] as const;
	});
...
export type NodeSelection = Readonly<
	{ nodes: Node[]; registers: Register[]; procRegisters: ReadonlyMap<number, number[]> }
>;
...
function remapNodes(
	...
	return nodes.map((x): Node => {
		if (x.op == "call") return { ...x, params: x.params.map(getReg) };
			...
			return {
				...x,
				conditional: x.conditional == null ? null : getReg(x.conditional),
				ref: typeof x.ref == "number" ? getNode(x.ref) ?? "unset" : x.ref,
			};
		} else if (x.op == "inc" || x.op == "dec") return { ...x, lhs: getReg(x.lhs) };
		else if (x.op == "add" || x.op == "sub" || x.op == "access" || x.op == "set") {
			return { ...x, lhs: getReg(x.lhs), rhs: getReg(x.rhs) };
		} else if (x.op == "setIdx") {
			return { ...x, lhs: getReg(x.lhs), rhs: getReg(x.rhs), idx: getReg(x.idx) };
		} else if (x.op == "breakpoint") {
			return { ...x, conditional: x.conditional == null ? null : getReg(x.conditional) };
		}
		return x.op; // never
	});
}
...
export function toSelection(procI: number, proc: Procedure, nodes: number[]): NodeSelection {
	...
	const newRegList = proc.registerList.filter(reg => regsUsed.has(reg)).map(reg =>
		[reg, proc.registers.get(reg)!] as const
	);
	...
	return {
		nodes: remapNodes(
			nodes.map(i => proc.nodes.get(i)!),
			x => newRegMap.get(x)!,
			x => nodeMap.get(x) ?? null,
		),
...
export function fromSelection(
	sel: NodeSelection,
	remapNode: number[],
	remapRegister: number[],
): [number, Node][] {
	return remapNodes(sel.nodes, x => remapRegister[x], x => remapNode[x]).map((
		x,
		i,
	) => [remapNode[i], x]);
}
...
export class InterpreterError extends Error {
	constructor(
		public value: { type: "badParam"; nParam: number; nProvided: number } | {
...
export function clone(prog: ProgramState) {
	return {
		...
		activeRegisters: undefined,
		stats: { ...prog.stats },
		outputRegister: { ...prog.outputRegister, mutable: prog.outputRegister },
		stack: prog.stack.map(v => ({
			...v,
			registers: new Map([...v.registers.entries()].map(([k, v]) => [k, { ...v, mutable: v }])),
		})),
	} as const;
}
...
export function push(prog: ProgramState, procI: number, params: RegisterRef[]) {
	...
	if (needed > params.length) {
		throw new InterpreterError({ type: "badParam", nParam: needed, nProvided: params.length });
	}
...
export const numToChar = Object.fromEntries(charMap.map(([a, b]) => [b, a])) as Record<
	number,
	string
>;
...
export function step(prog: ProgramState): "breakpoint" | boolean {
	...
	const compute = (op: "add" | "sub", l: RegisterRef, r: RegisterRef) => {
		...
		if (typeof l.current == "string" && typeof r.current == "bigint") {
			...
			out = op == "add"
				? l.current+r.current
				: l.current.slice(0, Math.max(0, l.current.length-r.current.length));
	...
	const arrOp = (idx: number, lhs: number, rhs?: number) => {
		...
		if (rhs == undefined) {
			get(idx).current = typeof l == "string"
				? BigInt(i >= l.length ? -1 : charToNum[l[i]] ?? -1)
				: l;
		} else {
	...
	let next = last.i+1;
	if (
		x.op == "breakpoint" && prog.stopOnBreakpoint
...
export type Verdict = Readonly<{ type: "RE" | "WA" | "AC" | "TLE" } & ProgramStats>;
	...
	const getStats = () => {
		...
		if (pstateStats.length > 0) {
			for (const k of ["nodes", "registers", "time"] as const) {
				...
				out[k] = Math.ceil(
					(pstateStats[Math.floor(pstateStats.length/2)][k]+pstateStats[
						Math.floor((pstateStats.length-1)/2)
					][k])/2,
				);
			}
		}
