...
export function toText({ procs, entry }: { procs: ReadonlyMap<number, Procedure>; entry: number }) {
	...
	const write = (i: number): string => {
		...
		const regNames = new Map(p.registerList.map((x, i) => {
			...
			inner.push(
				`\t${reg.type == "value" ? "register" : "parameter"} ${quote(regName)}${
					reg.type == "value"
						? ` = ${typeof reg.value == "string" ? quote(reg.value) : reg.value}`
						: ""
				}`,
			);
			...
			inner.push(
				`${i+1}\t${
					((x.op == "goto"
						...
						: x.op == "add" || x.op == "sub" || x.op == "access" || x.op == "set"
						? [x.op, get(x.lhs), get(x.rhs)]
						: x.op == "inc" || x.op == "dec"
						? [x.op, get(x.lhs)]
						: x.op == "setIdx"
						? ["set_idx", get(x.lhs), get(x.idx), get(x.rhs)]
						: x.op == "breakpoint"
						? ["breakpoint", ...x.conditional != null ? [get(x.conditional)] : []]
						: x.op == "call"
						? ["call", write(x.procRef), ...x.params.map(get)]
...
function fromTextErr(
	txt: string,
	maxProc: number,
): { procs: Map<number, Procedure>; entry: number | null; maxProc: number; procList: number[] } {
	const toks: Readonly<
		{ type: "register"; register: Register & { name: string } } | {
			type: "goto";
			num: number | "unset" | "end";
			cond: string | null;
		} | { type: "op"; op: Exclude<Node["op"], "goto">; params: (string | null)[] } | {
			type: "def";
			name: string;
			main: boolean;
		} | { type: "comment"; txt: string } | { type: "endDef" }
	>[] = [];
	...
	for (let line of txt.split("\n")) {
		...
		const parseNum = <Big extends boolean>(
			big: Big,
		): (Big extends true ? bigint : number) | null => {
		...
		if (take("// ")) {
			toks.push({ type: "comment", txt: line });
			line = "";
			...
			toks.push({
				type: "register",
				register: { type: "value", name, value: num == null ? parseStr() : num },
			});
		} else if (take("parameter ")) {
			toks.push({ type: "register", register: { type: "param", name: parseStr() } });
		} else if (take("}")) {
	...
	for (const tok of toks) {
		if (tok.type == "def") {
			...
			if (tok.main) {
				if (entry != null) {
					throw new TextParseError(
						`both ${quote(entry)} and ${quote(tok.name)} declared as main procedures`,
					);
				}
				entry = tok.name;
			} else {
			...
			if (tok.type == "goto") {
				...
				node = {
					op: "setIdx",
					lhs: get(tok.params[0]),
					idx: get(tok.params[1]),
					rhs: get(tok.params[2]),
				};
			} else if (tok.op == "breakpoint") {
				node = {
					op: "breakpoint",
					conditional: tok.params.length == 0 ? null : get(tok.params[0]),
				};
			} else if (tok.op == "call") {
				if (tok.params[0] == undefined) {
					throw new TextParseError("expected proc name for call operation");
				}
				node = {
					op: "call",
					procRef: getProcI(tok.params[0]),
					params: tok.params.slice(1).map(get),
				};
			} else {
	...
	return { procs, maxProc, procList, entry: entry != null ? procByName.get(entry) ?? null : null };
}

export function fromText(
	txt: string,
	maxProc: number,
): { type: "ok" } & ReturnType<typeof fromTextErr> | { type: "error"; error: string } {
	try {
...
export function addFromText(
	edit: EditorState,
	txt: string,
): { type: "ok"; value: EditorState } | { type: "error"; error: string } {
	...
	return {
		...
		value: {
			...edit,
			procs: newProcs,
			userProcList: [...edit.userProcList.filter(x => newProcs.has(x)), ...res.procList],
			maxProc: res.maxProc,
