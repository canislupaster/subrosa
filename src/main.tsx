...
import { IconBookFilled, IconBrandGithubFilled, IconChevronRight, IconCircleCheckFilled,
	IconCircleDashedCheck, IconDeviceDesktopFilled, IconDownload, IconPuzzleFilled, IconSkull,
	IconTriangleFilled, IconUpload } from "@tabler/icons-preact";
import { ComponentChildren, ComponentProps, createElement, Fragment, JSX, render } from "preact";
...
import { Anchor, anchorHover, anchorUnderline, bgColor, Button, ConfirmModal, Container, Divider,
	ease, GotoContext, Input, mapWith, Modal, setWith, Text, textColor, Theme, ThemeContext, throttle,
	useAsyncEffect, useFnRef, useGoto, useMd, useTitle } from "./ui";
...
function Footer() {
	return <div className={`mt-5 ${textColor.dim} pl-3 mb-10 flex flex-col items-center gap-2`}>
		<p>
			<span className="-ml-8" />
			A game handcrafted by{" "}
			<Anchor href="https://thomasqm.com" target="_blank">Thomas Marlowe</Anchor>,<br />{" "}
			<Anchor href="https://github.com/kartva" target="_blank">Kartavya Vashishtha</Anchor>, and
			{" "}
			<Anchor href="https://linkedin.com/in/peterjin25/" target="_blank">Peter Jin</Anchor>.
		</p>
		<p>
			<Anchor href="https://github.com/canislupaster/subrose"
				className={twJoin("items-center", textColor.dim)} target="_blank">
...
function Home() {
	const goto = useGoto();
	return <>
		<FadeRoute className="flex flex-col items-center pt-10 gap-2 max-w-xl px-5">
			<img src="/big.svg" />
			<Text v="big" className="text-center">A story-based puzzle game</Text>
			<Text v="md" className="text-center -mt-2">
				blending inductive reasoning, cryptography,<br /> and assembly programming.
			</Text>

			<Text className="mt-3">
				<b>Ready to join our team?</b> Apply for a summer internship today! <i>Anyone</i>{" "}
				with strong problem solving skills will excel in our fast-paced growth-oriented environment.
			</Text>

			<Button onClick={() => goto("/menu")} autofocus className="text-2xl px-4 py-3 border-2 my-4">
				Apply now
			</Button>
...
export const Logo = forwardRef<HTMLButtonElement, { className?: string; menu?: boolean }>(
	({ className, menu }, ref) => {
		...
		return <button onClick={() => goto(menu == true ? "/menu" : "/")}
			className={twMerge("w-1/2 self-end", className)} ref={ref}>
			<LogoBack />
		</button>;
	},
);
...
function Menu() {
	...
	return <>
		<FadeRoute className="flex flex-row items-stretch gap-20 pt-10 w-3xl">
			<div className="flex flex-row gap-5 mt-20 py-10">
				<div className="flex flex-col gap-1 items-end">
					...
					<div className={twJoin("mb-10", textColor.gray, bigTextClass)}>
						...
						<span
							className={twJoin(
								bgColor.divider,
								"h-[120%] mb-1.5 align-middle w-2 inline-block mx-2 -skew-x-6",
							)} />
				...
				<div className={twJoin(bgColor.default, "w-1 relative h-[90%]")}>
					<div
						className={twJoin(
							bgColor.divider,
							"absolute top-0 left-0 right-0 z-10 animate-[expand-down_2000ms_forwards] origin-top",
						)}
						style={{
							height: percentProgress,
							borderBottomLeftRadius: progressBorderRadius,
			...
			<div className="flex flex-col gap-4 grow">
				...
				}} msg={"Are you sure you want to clear your progress?"} open={confirmReset}
					...
					return <Fragment key={stageUrl(stage)}>
						{stage.type == "story" && stage.startOf != undefined
							&& <Text v="md" className="mt-1 -mb-2 flex flex-row gap-2 items-center">
								<IconTriangleFilled size={18} />
								{stage.startOf} <Divider className="w-auto ml-2 grow" />
							</Text>}
						<button
							className={twMerge(
								a && anchorHover,
								"flex flex-col gap-0.5 p-2 pt-1 group items-start relative pr-10",
							)}
							disabled={!a}
							...
							<div className="flex flex-row gap-2 items-center">
								{stage.done
									...
									: stage.i <= activeStages && <IconCircleDashedCheck />}
								<Text v="bold" className={twJoin(a ? anchorUnderline : textColor.dim)}>
									{stage.name}
								</Text>
								{createElement(icon, {
									className: "absolute -left-4 top-2 fill-white/15 -z-10",
									size: 50,
								})}
							</div>
							{stage.i <= activeStages
								&& <IconChevronRight size={36}
									className="transition-transform group-hover:translate-x-4 absolute top-2 right-4" />}
							<Text v="sm" className={twJoin(!a && textColor.dim)}>{stage.blurb}</Text>
						</button>
						{stage.i == activeStages-1 && stage.i < stages.length-1
							&& <div className="flex flex-row items-center gap-2 -my-2" ref={nextStageRef}>
				...
				<div className="flex flex-row w-full justify-between mt-2 -mb-5 self-start">
					<Anchor className={twJoin(textColor.red, "items-center")}
...
function PuzzleStage({ stage, i }: { stage: Stage & { type: "puzzle" }; i: number }) {
	...
	const setEdit2 = useCallback((cb: (old: EditorState) => EditorState) =>
		setEdit(old => {
			const ns = cb(old!);

			throttleSave.current?.call(() => {
				LocalStorage.stepsPerS = ns.stepsPerS;
				procStorage.setProcs([...ns.procs.entries()].map(([k, v]): [number, Procedure] => [k, v]));
...
function PuzzleStageWrap(props: ComponentProps<typeof PuzzleStage>) {
	...
	if (editingElsewhere) {
		return <ErrorPage errName="Don't open up multiple tabs!">
			You might impinge on the spatial-temporal consistency of your programs! Your progress might be
			nuked! I didn't design for this! What kind of game syncs progress between multiple windows?!
		</ErrorPage>;
	}
...
function StoryStage({ stage, i }: { stage: Stage & { type: "story" }; i: number }) {
	...
	return <FadeRoute className="w-4xl flex flex-row items-start gap-4 pb-[30dvh] pt-10">
		...
		<Logo
			className="basis-1/4 shrink-0 self-start sticky top-4 hover:opacity-100! transition-opacity duration-300"
			ref={logoRef}
			menu />
	</FadeRoute>;
}
		...
		{withDone.map(stage => {
			...
			return <Route key={path} path={path} component={() =>
				stage.i > activeStages
					? <LockedStage />
					: stage.type == "story"
					? <StoryStage i={stage.i} stage={stage} />
