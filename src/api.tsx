...
import { API, COUNT_PLAY_INTERVAL_SECONDS, parseExtra, ServerResponse, StageStatsResponse, statsArr,
	stringifyExtra, toPrecStat, validUsernameRe } from "../shared/util";
...
import { Alert, AlertErrorBoundary, bgColor, borderColor, Button, Collapse, Input, Loading, mapWith,
	setWith, Text, useAsync, useAsyncEffect, useToast } from "./ui";

export type Submission = { active: number; procs: ReadonlyMap<number, Procedure> };

export type APIRequest = {
	[K in keyof API]: [K, API[K]["request"]] | [
		K,
		API[K]["request"],
...
export function useAPI<T extends keyof API>({ propagateError }: { propagateError?: boolean } = {}) {
...
type SubmissionStatus = { type: "judging" } | { type: "done"; verdict: Verdict } | null;

type SubmissionWithStats = { sub: Submission; stats: ProgramStats };

function Leaderboard(
	{ submission: s, puzzle }: { submission: SubmissionWithStats | null; puzzle: Puzzle },
) {
	const [sort, setSort] = useState<keyof ProgramStats>("time");
	const [input, setInput] = useState(LocalStorage.username ?? "");

	const [data, setData] = useState<
		{ type: "data"; tokenId: PuzzleSolveData } | { type: "nosolve" } | null
	>(null);
...
export function useSubmission(
	...
	return {
		...
		status,
		loading,
		resubmitting: s != null || alreadySolved,
		alreadySolved,
		puzzle,
		nextStage,
		setSubmission: setS,
...
export function Submission(
	...
	if ((status?.type == "done" && status.verdict.type == "AC") || (!status && alreadySolved)) {
		...
		inner = <Alert bad title="Test failed"
			txt={`Verdict: ${
				({ WA: "wrong answer", RE: "runtime error", TLE: "time limit exceeded" } as const)[
					status.verdict.type
				]
			}`} />;
	}
