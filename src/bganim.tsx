...
class BgAnim extends DisposableStack {
	...
	async unwrite() {
		...
		for (const c of this.root.children) {
			proms.push(
				c.animate([{ opacity: 1 }, { opacity: 0 }], { duration: 50, delay }).finished.then(() =>
					c.remove()
				),
			);
...
export function BgAnimComponent() {
	...
	return <pre ref={ref}
		className={twJoin(
			"overflow-hidden flex flex-row flex-wrap items-start fixed left-0 right-0 top-0 max-h-dvh w-[calc(100%+5rem)] transition-colors text-xl -z-10 duration-1000",
			unwriting ? "text-zinc-300" : "text-gray-600/10",
		)} />;
}
