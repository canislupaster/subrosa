...
async function loadImage({
	...
	for (const x of arr) {
		...
		out.value.push(x[2]);
		out.html.push(`<span style="color: ${bestHex};" >${bestChar[Math.round(x[2]*x[2]*100)]}</span>`);
	}
	
	return out;
}

export function AsciiArt({src, ...opts}: Partial<Record<keyof ImageProps, string|number>>&{src: string}) {
	const disableAnim = useDisableStoryAnimation();
	const [img, setImg] = useState<Awaited<ReturnType<typeof loadImage>>|null>(null);
	useAsyncEffect(async ()=>{
		setImg(await loadImage({
			...Object.fromEntries(Object.entries(opts).map(([k,v])=>[
	...
	useEffect(()=>{
		...
		const frame = ()=>requestAnimationFrame((t: number)=>{
