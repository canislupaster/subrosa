...
import { AddSolve, API, CountStage, parseExtra, ServerResponse, StageStats, stringifyExtra,
	validUsernameRe } from "../shared/util.ts";
import { addSolve, countPlay, getStats, setUsername } from "./db.ts";

export class AppError extends Error {
	constructor(public msg: string, public status: StatusCode = 500) {
		super(msg);
	}
}
...
const stageReq = z.object({ stage: z.string() }) satisfies z.ZodType<CountStage>;

const statsReq = stageReq.and(
	z.object({ orderBy: z.literal("time").or(z.literal("nodes")).or(z.literal("registers")) }),
...
}).or(z.object({ type: z.literal("param"), name: z.string().nullable() }));

const node = z.union([
	z.object({ op: z.enum(["inc", "dec"]), lhs: z.number() }),
	z.object({
		op: z.literal("goto"),
		conditional: z.number().nullable(),
		ref: z.union([z.number(), z.literal("unset"), z.literal("end")]),
	}),
...
function getPuzzle(puzzleKey: string) {
	const ret = data.find(x => x.type == "puzzle" && x.key == puzzleKey) as StageData & {
		type: "puzzle";
	} | undefined;
...
makeRoute<"solve">({
	...
	async handler(solve) {
		...
		const worker = new Worker(new URL("../shared/worker.ts", import.meta.url).href, {
			type: "module",
		});
...
makeRoute<"setusername">({
	...
	async handler({ token, username }) {
		await setUsername(token, username);
	},
});
